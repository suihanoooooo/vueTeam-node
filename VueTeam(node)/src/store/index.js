...
const mutations = {
	LOADOBJ (state,arr) {
       arr.forEach(function(dom,index,arr){
          if(dom.objClass=='内部项目'){
                state.neibu.Objects.push(dom);
          }
          else if (dom.objClass=='商户接入') {
                state.shanghu.Objects.push(dom);
          }
          else if (dom.objClass=='渠道接入') {
                state.qudao.Objects.push(dom);
          }
       });
	},
